...
import { Uni3Snaphot } from '../../../lib/datasource/univ3Arbitrum.js';
...
const Log = new Measurement<ILogAny, any, any>('ssc_weth_strategy');
...
class SingleSidedUniswap {
  ...
  public async end(uni: UniV3PositionManager, data: Uni3Snaphot) {
    ...
    console.log(`fees: ${this.series[this.series.length-1].fees}`);
...
export class SingleSidedUniswapStrategy {
  ...
  constructor() {
    const strategies = Array.from(Array(3).keys()).map((i) => {
      const n = i + 1;
      return {
        initialInvestment: 100_000,
        name: `#${80 + n}: UNI3-WETH/USDC ${80 + n}%`,
        pool: 'Univ3 WETH/USDC 0.05%',
        rangeSpread: 0.80 + (0.01 * n),
        priceToken: 0,
      };
    });
